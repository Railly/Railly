...
function Logo({
	...
	const parsedLogo = useMemo(() => (raw ? svgInner(raw) : null), [raw]);
	const innerGroupRef = useRef<SVGGElement | null>(null);

	useEffect(() => {
		if (innerGroupRef.current) {
			innerGroupRef.current.innerHTML = parsedLogo?.inner ?? "";
		}
	}, [parsedLogo]);

	if (parsedLogo) {
		const { viewBox } = parsedLogo;
...
function xToPrice(x: number): number {
	...
	return Math.round(
		Math.exp(
			Math.log(PRICE_MIN) + t * (Math.log(PRICE_MAX) - Math.log(PRICE_MIN)),
		),
	);
}
	...
	return (
		<div className="relative flex flex-col border border-ui rounded bg-background">
			...
			<div className="px-6 pb-2 text-xs font-mono text-foreground-2 flex items-center gap-2">
				<span className="text-foreground-3 uppercase tracking-wider">
					Budget
				</span>
