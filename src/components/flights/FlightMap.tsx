...
import { cityCoordinates, projectCity, southAmericaBounds } from '../../utils/geoUtils';
...
const projectCities = (): Record<string, City> => {
  ...
  Object.entries(cityCoordinates).forEach(([key, coords]) => {
    const projected = projectCity(coords.lon, coords.lat, southAmericaBounds, PROJECTION_CONFIG);
...
export default function FlightMap({ flights }: FlightMapProps) {
    ...
    return polygons.map(polygon => {
      return polygon.map(ring => {
        return ring.map((coord, i) => {
          const [lon, lat] = coord;
          const projected = projectCity(lon, lat, southAmericaBounds, PROJECTION_CONFIG);
  ...
  return (
    <div className="relative w-full">
      ...
      <div className="relative bg-flexoki-bg-2 rounded-lg border border-flexoki-ui overflow-hidden">
        ...
        >
          ...
          {flights.map((flight, index) => {
            ...
            const isActive = hoveredCity === flight.to || selectedCity === flight.to;
          ...
          {Object.entries(cities).map(([cityKey, city]) => {
            const isVisited = flights.some(f => f.to === cityKey) || city.isHome;
            if (!isVisited) return null;

            const isActive = hoveredCity === cityKey || selectedCity === cityKey;
          ...
          >
            <div className="flex items-start justify-between mb-2">
              <div>
                ...
                <p className="text-xs text-flexoki-tx-3 font-mono">{selectedFlight.date}</p>
              </div>
              <button
      ...
      {/* Legend */}
      <div className="mt-4 flex items-center justify-center" style={{ gap: '1rem' }}>
        ...
        <div className="flex items-center text-xs text-flexoki-tx-3" style={{ gap: '0.5rem' }}>
          <svg width="16" height="16" viewBox="0 0 24 24" stroke="var(--color-flexoki-tx-3)" strokeWidth="2" fill="none">
