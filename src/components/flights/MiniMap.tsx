...
export default function MiniMap({ totalCountries, visitedCountries, totalFlights }: MiniMapProps) {
  const visitedPercentage = Math.round((visitedCountries / totalCountries) * 100);
    ...
    return polygons.map(polygon => {
      return polygon.map(ring => {
        return ring.map((coord, i) => {
          const [lon, lat] = coord;
          const projected = projectCity(lon, lat, southAmericaBounds, MINI_PROJECTION_CONFIG);
  ...
  const limaCoords = projectCity(-77.0428, -12.0464, southAmericaBounds, MINI_PROJECTION_CONFIG);
    ...
    <div className="relative bg-flexoki-bg-2 rounded-lg border border-flexoki-ui p-4 overflow-hidden">
      <div className="flex items-center justify-between" style={{ gap: '1.5rem' }}>
        ...
        <div className="flex-shrink-0">
          <svg width="80" height="120" viewBox="0 0 500 700" className="opacity-60">
        ...
        <div className="flex-1 grid grid-cols-2" style={{ gap: '1rem' }}>
          ...
          <div className="col-span-2">
            <div className="flex items-center justify-between mb-1">
              <span className="text-xs text-flexoki-tx-3">South America Coverage</span>
              <span className="text-xs font-mono text-flexoki-tx-2">{visitedPercentage}%</span>
            </div>
